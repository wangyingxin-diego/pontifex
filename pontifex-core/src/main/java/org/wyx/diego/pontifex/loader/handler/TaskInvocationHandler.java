...
public TaskInvocationHandler(Task proxyed, TaskRuntimeObject runtimeObject) {
        super(runtimeObject, new TaskInvocationHandler.TaskInvoker(proxyed), proxyed);
        }

public static class TaskInvoker implements Invoker {
    private Task proxyed;
    ...
    public Object before(InvokerParam invokerParam) {
        ...
        if (!method.getName().equals("run")) {
            TaskContext var5 = (TaskContext)args[0];
        }
